...
const Main = ({ searchValue, cart, setCart }) => {
  ...
  const handlerSearchColors = (id) => {
    if (search.colours.includes(id)) {
      setSearch({
        ...search,
        colours: search.colours.filter((colorId) => colorId !== id),
      });
    } else {
  ...
  const addToCart = () => {
    ...
    let isDiscountPrice =
      item.article.discount === null
        ? item.article.price
        : item.article.discount; // Ako je svojsvto discount razlicito od null onda postoji popust
  ...
  return (
    <div className="wrapper">
      <button
        className="open-filter-small"
        onClick={() => openFilterMenuSmall()}
      >
        FILTER
      </button>
            ...
            {searchData.brands.map((brand, index) => (
              <div key={index} className="checkbox-custom">
                <input
                  type="checkbox"
                  id={`brand${index}`}
                  checked={search.brands.includes(brand)}
                  onChange={() => handlerSearchBrands(brand)}
                />
            ...
            {searchData.colours.map((colour, index) => (
              <div key={index} className="checkbox-custom">
                <input
                  type="checkbox"
                  id={`colour-${index}`}
                  checked={search.colours.includes(colour)}
                  onChange={() => handlerSearchColors(colour)}
                />
        ...
        <div className="article-size-container">
          ...
          <div className="article-size-container-list">
            {searchData.sizes.map((size, index) => {
              return (
                <div
                  onClick={() => handlerSearchSize(size)}
                  key={index}
                  className="size-box"
                  style={{
                    backgroundColor: search.size === size ? "#98002e" : "white",
                    color: search.size === size ? "#f9a01b" : "black",
                  }}
                >
                  <p>{size}</p>
                </div>
      ...
      <div className="main">
        {articles.map((article) => {
          if (
            ...
            !search.brands.includes(article.brand)
          ) {
            return null;
          }

          if (
            search.colours.length > 0 &&
            !search.colours.includes(article.colour)
          ) {
            return null;
          }
          ...
          return (
            ...
            >
              ...
              <div className="price-container">
                {!article.discount ? (
                  <p className="price">{article.price} RSD</p>
                ) : (
                  <p className="price slashed">{article.price} RSD</p>
                )}
                {!article.discount ? (
                  <span></span>
                ) : (
                  <p className="discount">{article.discount} RSD</p>
                )}
              </div>
            </div>
          );
                      ...
                      {item.article.sizes.map((article, i) => {
                        return (
                          <div
                            ...
                            style={{
                              backgroundColor:
                                article === item.size ? "#98002e" : "white",
                              color:
                                article === item.size ? "#f9a01b" : "black",
                            }}
                            article-checked={article}
                            onClick={() => setItem({ ...item, size: article })}
                          >
                            <p>{article}</p>
                          </div>
                        );
                  ...
                  <div>
                    <button className="modal-order" onClick={addToCart}>
                      {!item.article.discount ? (
                        <p className="discount">{item.article.price} RSD</p>
                      ) : (
                        <p className="discount">{item.article.discount} RSD</p>
                      )}
                    </button>
                  </div>
                </div>
